...
@section('filter')
        ...
        <div>
                <form id="categoryForm" action="{{ route('offre.create') }}" method="get" class="rounded-lg">
                    @csrf

                <select name="category" id="category" class="bg-transparent hover:bg-white text-gray-400 hover:text-slate-950  border-gray-300 focus:border-indigo-500   shadow-sm w-full border-0 border-b-2  focus:ring-0 rounded-3xl"  onchange="this.form.submit()">
                ...
                <div >
                    <form class="" id="regionForm" action="{{ route('offre.create') }}" method="get">
                    <select name="region" class="bg-transparent text-gray-400 hover:text-slate-950 border-gray-300 focus:border-indigo-500 hover:bg-white    shadow-sm w-full border-0 border-b-2  focus:ring-0 rounded-3xl" onchange="this.form.submit()" >
 ...
 <div id="work-offers-list" class="mx-4 colored lg:grid lg:grid-cols-4 gap-4 space-y-4 md:space-y-0 sm:grid-cols-2 md:grid-cols-3  xl:grid-cols-5  grid grid-cols-auto-fit ">
      ...
      @foreach($offres as $offre)
        ...
        <x-card id="card" class="p-10 bg-blue-200 hover:bg-blue-500" style="border-radius: 10%">
